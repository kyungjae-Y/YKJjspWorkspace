	...
	<c:if test="${vo ne null}">
		...
		<form action="${ctx}/memberUpdate.do" method="post">
			...
			<table class='table table-bordered'>
				...
				<tr>
					<td colspan="2" align="center"><input type="button" value="사진 업로드" class='col-3 btn btn-success' id="uploadBtn" <c:if test="${loginId!=vo.id and loginId!='admin'}"> 
    	              disabled="disabled"
          	      </c:if> /> <input type="submit" value="수정하기" class='col-3 btn btn-primary' <c:if test="${loginId!=vo.id and loginId!='admin'}"> 
    	              disabled="disabled"
          	      </c:if> /> <input type="button" value="사진 삭제" class='col-3 btn btn-warning' id="deleteBtn" <c:if test="${loginId!=vo.id and loginId!='admin'}"> 
    	              disabled="disabled"
          	      </c:if> /></td>
				</tr>
			</table>
